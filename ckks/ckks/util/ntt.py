...
class NTTContext:
    ...
    Here, R is the quotient ring Z_q[x]/f(x), where f(x) = x^d + 1.
    The NTTContext keeps track of the ring degree d, the coefficient
    modulus q, and a root of unity w so that w^2d = 1 (mod q), and
    ...
    def __init__(self, poly_degree: int, coeff_modulus:int , root_of_unity=None):
        """Inits NTTContext with a coefficient modulus for the polynomial ring
        Z[x]/f(x) where f has the given poly_degree.

        Args:
            poly_degree (int): Degree of the polynomial ring. = N = d
            coeff_modulus (int): Modulus for coefficients of the polynomial.
    ...
    def precompute_ntt(self, root_of_unity: int):
        ...
        self.roots_of_unity = [1] * self.degree # degree = N = d
        ...
        Args:
            ...
            rou (list): Powers of roots of unity to be used for transformation.
                For inverse NTT, this is the powers of the inverse root of unity.
                    = self.roots_of_unity = [w^0, w^1, ..., w^d]
        Returns:
