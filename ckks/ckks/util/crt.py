...
import ckks.util.number_theory as nbtheory
from ckks.util.ntt import NTTContext
...
class CRTContext:
    ...
    Attributes:
        ...
        crt_vals[i] = modulus / prime_i
        crt_inv_vals[i] = crt_vals[i]^-1 mod prime_i
    """

    def __init__(self, num_primes: int, prime_size: int, poly_degree: int):
        """Inits CRTContext with a list of primes.

        Args:
            num_primes (int): Number of primes.
            prime_size (int): Minimum number of bits in primes.
            poly_degree (int): Polynomial degree of ring = d = N.
        """
    ...
    def primes_to_c_include(self):
        """
        prints the current primes to the shell, in a format which is readable by a C compiler
        Returns: nothing
        """
        assert self.primes
        print("const uint64_t primes = {", end='')
        print(", ".join([str(p) for p in self.primes]), end='')
        print("};")
