...
impl Number<StubGlobalState> for rug::Float {
    fn rand_normal(n: usize, exp_from: i32, exp_to: i32, _gs: StubGlobalState, sign_positive: bool) -> Vec<Self> {
        ...
        for _ in 0..n {
            ...
            let f = Float::with_val(prec,Float::random_bits(&mut rand));
            let p = Float::with_val(prec, Float::i_pow_u(10, exp.unsigned_abs()));
            ret.push(Float::with_val(prec, if exp >= 0 { f*p*sign } else { f/p*sign }));
        }
        ret
    }
